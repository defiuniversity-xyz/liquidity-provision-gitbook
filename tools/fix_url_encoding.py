...
BUCKET_NAME = "liquidity-provision-media"
...
def fix_url_encoding(lesson_file: Path) -> tuple[bool, str]:
    ...
    # Pattern to match embed tags with liquidity-provision-media URLs
    pattern = r'\{% embed url="(https://storage\.googleapis\.com/liquidity-provision-media/[^"]+)" %\}'
    ...
    if matches:
        ...
        return False, f"No liquidity-provision-media embeds found in {lesson_file.name}"
